...
@DataJpaTest(properties = {
        ...
        "spring.datasource.url=jdbc:h2:mem:course-difficulty-repository;MODE=PostgreSQL;DB_CLOSE_DELAY=-1;DATABASE_TO_LOWER=TRUE;INIT=CREATE DOMAIN IF NOT EXISTS JSONB AS JSON",
...
@DisplayName("코스 난이도 리포지토리")
class CourseDifficultyRepositoryTest {
    ...
    @DisplayName("활성 코스 난이도를 QueryDSL fragment로 조회한다")
    @Test
    void findsActiveDifficultyByCourseId() {
        assertThat(courseDifficultyRepository).isInstanceOf(CourseDifficultyCustomRepository.class);

        Course course = saveCourse("difficulty-course");
        entityManager.persist(CourseDifficulty.create(course, Difficulty.HARD));
        entityManager.flush();
        entityManager.clear();

        assertThat(courseDifficultyRepository.findDifficultyByCourseId(course.getId()))
                .contains(Difficulty.HARD);
    }

    @DisplayName("삭제된 코스 난이도는 조회하지 않는다")
    @Test
    void doesNotFindDeletedDifficulty() {
        Course course = saveCourse("deleted-difficulty-course");
        CourseDifficulty courseDifficulty = CourseDifficulty.create(course, Difficulty.EASY);
        courseDifficulty.delete();
        entityManager.persist(courseDifficulty);
        entityManager.flush();
        entityManager.clear();

        assertThat(courseDifficultyRepository.findDifficultyByCourseId(course.getId())).isEmpty();
    }
